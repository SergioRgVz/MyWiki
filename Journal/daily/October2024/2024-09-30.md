...
def create_coco_annotations(image_paths, coco, all_results, output_file):
    coco_output = {
        "info": {
    ...
    # Add categories
    if all_results and len(all_results) > 0 and 'categories' in all_results[0][0]:
        for i, category in enumerate(all_results[0][0]['categories'], 1):
    ...
    annotation_id = 1
    for image_id, (image_path, results) in enumerate(zip(image_paths, all_results), 1):
        ...
        coco_output["images"].append({
            ...
            "license": coco.loadImgs(img_id)[0]['license'],
            "flickr_url": coco.loadImgs(img_id)[0]['flickr_url'],
            "coco_url": coco.loadImgs(img_id)[0]['coco_url'],
            "date_captured": coco.loadImgs(img_id)[0]['date_captured']
        })

        for result in results:
            if not result:
                continue
            masks_yolo = result.get('masks', [])
            boxes = result.get('boxes', []).tolist()
            cls = result.get('classes', []).tolist()
            conf = result.get('confs', []).tolist()
            height, width = result['size']['height'], result['size']['width']
            left, top, right, bottom = result['relative_position']['left'], result['relative_position']['top'], result['relative_position']['right'], result['relative_position']['bottom']
    ...
    for image_file in os.listdir(IMAGE_DIR):
        if image_file.lower().endswith(('.png', '.jpg', '.jpeg')):
            ...
            if result:
                ...
                all_results.append(result['all_results'])
                coco = result['coco']
                image_paths.append(image_path)
            print("---------------------------")
    
    # Create a single COCO annotation file for all processed images
    output_file = os.path.join(OUTPUT_DIR, 'all_annotations.json')
    create_coco_annotations(image_paths, coco, all_results, output_file)
    
    # visualized_image = visualize_coco_annotations(image_paths[0], output_file)
    # cv2.imwrite(os.path.join(OUTPUT_DIR, 'visualized_coco_annotations.jpg'), cv2.cvtColor(visualized_image, cv2.COLOR_RGB2BGR))
#         cv2.imwrite(os.path.join(output_dir, f'visualized_coco_annotations_{image_name}'), cv2.cvtColor(visualized_image, cv2.COLOR_RGB2BGR))

    print(f"All images processed. COCO annotations saved to: {output_file}")
...
def create_coco_annotations(output_file):
    coco_output = {
        "info": {
            "description": "Dataset generated with YOLOv8",
            "url": "",
...
def update_coco_annotations(output_file, image_data, results, coco):
    with open(output_file, 'r+') as f:
        coco_output = json.load(f)

        image_id = len(coco_output["images"]) + 1
        img_id = int(image_data['file_name'].split('.')[0])
        ...
        if results and 'categories' in results[0]:
            for i, category in enumerate(results[0]['categories'], 0):
                category_id = 81 + i
                if not any(cat['id'] == category_id for cat in coco_output["categories"]):
                    coco_output["categories"].append({
                        "id": category_id,
                        "name": category,
                        "supercategory": ""
...
def main():
    ...
    IMAGE_DIR = '/home/COCO/train2017'
    ANNOTATIONS_PATH = '/home/COCO/annotations_trainval2017/annotations/instances_train2017.json'
    MODEL_PATH = "/home/COCO/scripts/segment_clothes.engine"
    OUTPUT_DIR = 'extracted_segments'
    ...
    output_file = os.path.join(OUTPUT_DIR, 'all_annotations.json')
    create_coco_annotations(output_file)
    ...
    for image_file in os.listdir(IMAGE_DIR):
        if image_file.lower().endswith(('.png', '.jpg', '.jpeg')):
            ...
            if result:
                ...
                print(f"  People with bags detected: {result['people_with_bags']}")
                print(f"  Full segmentation saved: {result['full_segmentation_path']}")
                update_coco_annotations(output_file, {'file_name': image_file}, result['all_results'], result['coco'])
            print("---------------------------")
...
if __name__ == "__main__":
    main()>)
```
[208.7178430557251,213.4272003173828,53.751227378845215,79.99734497070312]

- [x] Convertir este script para que vaya cogiendo una a una las imágenes y vaya buscando en el dataset de coco una a una, porque se está rayando muchísimo solo de recorrer el json de COCO
- [ ] Limpiar dataset para obtener nuevos ficheros para entrenar
- [ ] Lanzar entrenamiento con los apuntes hechos en Google Tasks
- [ ] Empezar curso de machine learning a las 8